...
-spec best(features(), examples(), number(), #rr_example{}, Conf::#rf_tree{}, integer(), integer(), score_fun()) -> #candidate{}.
...
best_rule(Features, Examples, Total, ExConf, Conf, NoFeatures, N, RuleScore, Cand) ->
    ...
    best_rule(Features, Examples, Total, ExConf, Conf, NoFeatures, N - 1, RuleScore,
              case NewCand#candidate.score < Score of
                  true ->
                      NewCand;
                  false ->
                      Cand
...
-spec generate_rule(features(), examples(), number(), #rr_example{}, Conf::#rf_tree{}, number(), score_fun()) -> #candidate{}.
generate_rule(Features, Examples, Total, ExConf, Conf, NoFeatures, RuleScore) ->
    ...
    #candidate{feature=Rule, score=Score(ExSplit, Total), split=ExSplit}.
...
learn_one_rule(Features, Examples, Total, Score, ExConf,  Conf) ->
    ...
    case rr_example:best_split(ExConf, Features, Examples, Total, Score, Split, Distribute, Missing) of
        no_features ->
            1;
        #candidate{feature=_Feature, split={_, _AnyExamples}, score={_Score, _, _}} ->
